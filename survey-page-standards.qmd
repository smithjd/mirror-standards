---
title: "survey-page-conventions"
subtitle: "Standards derived from the `survey-page-conventions` skill"
---
...
## Visual Identity: Background Tints
...
## ggplot Template: Horizontal Bar (percentages)
...
**Scale note:** Survey percentages are often stored as 0–1 proportions — use `scales::percent_format()` and `scales::percent(pct, accuracy = 1)`.
If stored as 0–100 (e.g., after `* 100`), use `scales::percent_format(scale = 1)` and `scales::percent(pct, accuracy = 1, scale = 1)`.

## ggplot Template: Stacked Bar (multi-response fill)
...
## Key Utility Functions (shared_functions_26.R)

### role_subset() — filter by role flag, get counts
```{.r}
...
### count_yes_str() — count binary Yes (==1) across role vars
```{.r}
...
### calc_pct_selected() — % selecting follow-up items among role-holders
```{.r}
...
### calc_val_labels() — build long-format count df for multi-item series
```{.r}
...
### plot_1_level() — quick horizontal bar for a single categorical var
```{.r}
...
### plot_interest() — horizontal bar for interest/support items
```{.r}

plot_interest(teacher_interest_counts, gtitle = "Are you interested in...?")
```

### count_all_that_apply() — multiple-select ("check all that apply") questions
```{.r}
...
### Counting Yes (==1) across non-contiguous named variables
...
### Extracting joinable variable labels for a named set of columns
...
### Deriving a short_label from verbose SPSS variable label text
Variable labels often share a common preamble (e.g., "In the past 12 months, did you ...?").
Extract just the differentiating fragment with a lookbehind/lookahead regex, then recode to
human-friendly labels. Use **`replace_values()`** when you only need to fix a few labels and
want all other values to pass through unchanged:
```{.r}
...
```
Use **`case_match()`** instead when building an entirely new vector and enumerating all cases
(any omitted value becomes `NA` unless `.default` is set):
```{.r}
...
Adjust the lookbehind string to match the preamble in your question series.
Note: `dplyr::recode()` is deprecated — use `replace_values()` or `case_match()` instead.

### disp_var_prep() — select a variable group by prefix
```{.r}
...
### pivot_df() — pivot to long format with within-variable percentages
```{.r}
...
### q0080 — Decade first joined (always collapse)
...
### q0032 — Relates to a centre (strip parenthetical label)
...
### q0034 — Primary membership identity (filter to centre-affiliates)
...
## Color Palette — When to Use `scale_fill_brewer`
...
## Standard Sub-population Filters
...
## Chunk Label Prefixes

- `fig-` for figures 
- `tbl-` for tables
- Descriptive snake_case after the prefix

## show_open_endeds Flag
