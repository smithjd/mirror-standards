# 06 — Implementation: R / ggplot2 / Quarto (non-normative appendix)

Maps the standard onto a ggplot2 (≥ 4.0) + Quarto stack. The normative rules live in
documents 01–05; this file shows one conforming implementation. Patterned on the BBC's
`bbplot`/cookbook approach.

## Palettes as constants

```r
# 03-color-palettes.md — categorical default (UK Analysis Function order)
...
Highlight-vs-context pattern (CO-5, CT-A7): map the featured series to
`palette_highlight[["highlight"]]` and all others to `palette_highlight[["context"]]`,
then label directly with `geom_text()` / `ggrepel` instead of a legend.

Small multiples (CT-3): `facet_wrap(vars(category))` with fixed scales unless a
documented reason requires `scales = "free_y"`.
...
- **Alt text (AX-6/7)** — every figure chunk sets `fig-alt`:
  ...
  ```{r}
  #| fig-alt: >
  #|   Line chart showing UK inflation falling from 11% in 2022
  #|   to 2% by mid-2024.
  plot_inflation
  ```
  ````

- **HTML-native titles (TY-5, AX-8)** — for web output, prefer the page's heading and
  lead paragraph to carry title/context, and emit SVG: `knitr: opts_chunk: dev: "svglite"`.
- **Theme discipline** — define `theme_report()`, palettes, and scale helpers once in
  a sourced `R/theme.R` (or an internal package), never inline per chunk; set
  `ggplot2::theme_set(theme_report())` in a setup chunk.
- **Brand** — a `_brand.yml` can carry the palette for non-ggplot outputs (tables,
  value boxes) so HTML components and charts share hex codes.
...
Colorblind simulation: `colorblindr::cvd_grid(plot_inflation)` or
